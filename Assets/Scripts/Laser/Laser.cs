...
public class Laser : MonoBehaviour
{
    ...
    [SerializeField] private float currentLength = 4f;     // 最長長度
    [SerializeField] private float width = 2f;     // 最長長度
    [SerializeField] private float growthRate = 2f;     // 長度增長速度
    ...
    private void Start()
    {
        Vector3 laserStartPosition = transform.position + transform.forward * 6f;
        transform.position = laserStartPosition;
        // 初始縮放為 1, 1, 0 (Z軸初始長度為 0)
    ...
    private IEnumerator ExpandLaser()
    {
        ...
        {
            // 控制雷射長度
            currentLength = Mathf.Min(maxLength, currentLength + growthRate * Time.deltaTime);
            transform.localScale = new Vector3(1f, currentLength, width); 
